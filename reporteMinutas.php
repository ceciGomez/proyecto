...
header("Content-Encoding: None", true);
        ...
        $fechaIngresoDesde=new DateTime();
        $fechaIngresoHasta=new DateTime();
        $fechaIngresoDesde= date_format($fechaIngresoDesde->createFromFormat('d/m/Y',$_GET['fechaIngresoDesde']), 'Y-m-d');
        $fechaIngresoHasta=date_format($fechaIngresoHasta->createFromFormat('d/m/Y',$_GET['fechaIngresoHasta']), 'Y-m-d');
...
function Header()
{
   // Logo
    ...
    $this->SetFont('Arial','B',15);
    $this->SetFont('','U');
    $this->Cell(100,10,'Lista de Minutas desde: '.$this->sanitizarFechaF($_GET['fechaIngresoDesde']).' hasta: '.$this->sanitizarFechaF($_GET['fechaIngresoHasta']),0,0,'C');
    $this->SetFont('','');
    // Salto de línea
    $this->Ln(15);
}
...
$consulta = mysqli_query($conexion,"
      ...
      , p.nroMatriculaRPI as nroMatriculaRPI, p.planoAprobado as planoAprobado,
                    l.nombre as localidad
                        ...
                        where cast(m.fechaIngresoSys as DATE)
                         between '".$fechaIngresoDesde."' and '".  $fechaIngresoHasta."'                   
